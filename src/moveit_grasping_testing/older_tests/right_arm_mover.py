...
#         self.right_arm_mgc.set_pose_reference_frame('base_link')
    ...
    def create_move_group_pose_goal(self, goal_pose=Pose(), group="right_arm", end_link_name="arm_right_tool_link", plan_only=True):
        ...
        while True:
            ...
            if not self.read_message:
                ...
                if self.last_hydra_message.paddles[1].buttons[0] == True:
                    ...
                    rospy.loginfo("Sending current right hand position & orientation")
                    moveit_goal = self.create_move_group_pose_goal(self.tmp_pose_right.pose, group="right_arm", end_link_name="arm_right_tool_link", plan_only=False)
...
if __name__ == '__main__':
    rospy.init_node('hydra_right_arm')
#    if len(sys.argv) < 2:
